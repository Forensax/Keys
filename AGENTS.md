...
用户通常会直接给出中文需求或完整实施计划。默认按计划执行，不停留在建议；遇到歧义时先结合上下文做保守判断，只有会破坏数据或无法确认目标时再提问。回复和文档优先使用中文，说明要直接、可执行。用户希望 Agent 像当前 Codex 一样：先读代码和测试，理解现有模式，再小步修改、验证、提交、推送、同步和重建 Docker。
