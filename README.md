...
- 支持单站监控任务，可在中转站恢复可用时发送 Telegram 通知。
...
导航中的“监控”页面用于创建单站可用性监控任务，并配置全局 Telegram Bot Token、Chat ID、启用状态和可选代理。每个监控任务绑定一个中转站，单独保存模型、客户端模式、网络路径、检测间隔、重试设置和通知类型；已禁用但未归档的中转站仍会检测，归档或删除的站点会跳过且不会发送外部请求。

监控检测结果写入独立的 `monitoring_checks`，不会写入普通 `connectivity_tests`，因此不会影响首页测试历史、定时任务历史或统计页数据。通知类型可选择“恢复可用时通知”和“变为不可用时通知”，至少启用一种；同一个成功或失败周期内不会重复通知，状态切换后才会再次通知。

监控任务和定时任务共用后台密钥授权机制。首次启用或立即运行前需要输入应用密码授权；“锁定后台密钥”会停用全部后台任务并清空下次执行时间。Telegram Token 和 Chat ID 使用应用密钥加密保存，页面不会回显明文。
...
JSON 备份版本 9 会导出定时任务、监控任务定义和目标映射，但不会导出后台包装密钥、任务运行历史、监控检测历史或完整测试历史。导入时按分组名称以及站点名称 + Base URL 重新映射目标，所有导入的定时任务和监控任务都会保持停用，必须重新授权并由用户手动启用。旧版本 JSON 备份仍可继续导入。

默认备份只记录 Telegram 是否已配置以及代理名称，不包含 Bot Token 或 Chat ID。勾选“包含明文 API Key 和代理认证信息”并再次确认应用密码后，备份会额外包含 Telegram 明文凭据，请按明文密钥文件处理。
...
Codex 模式会按当前 Codex Desktop 的 Responses 协议发送流式请求，包括结构化消息、`prompt_cache_key`、session/thread/turn/window 关联信息和客户端元数据。测试请求不启用工具，会发送一道随机中文简单计算题；系统会从 SSE 响应中提取实际文本并保存到测试结果。若错误中仍出现 `detected: Go-http-client/1.1`，说明中间层没有把 Codex 客户端 Header 转发给最终上游，需要由该中转站管理员启用请求头透传。
