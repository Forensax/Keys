...
- 提供测试统计仪表盘，支持真实历史与不落库的演示数据、时间/站点/来源筛选、成功率、延迟趋势、站点排行和失败原因分析。
...
演示模式会在每次请求中确定性生成 30 天、6 个示例站点的数据，只用于预览图表，不会新增、修改或删除数据库记录。页面图表使用项目内的 HTML、CSS、SVG 和 Vanilla JavaScript，不依赖 CDN；每个图形标记都提供键盘焦点和数值提示，同时保留站点表现表格供精确读取。
