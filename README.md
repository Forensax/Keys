...
导航中的“监控”页面用于创建单站可用性监控任务，并配置全局 Telegram Bot Token、Chat ID、启用状态和可选代理。每个监控任务绑定一个中转站，单独保存模型、客户端模式、网络路径和检测间隔；已禁用但未归档的中转站仍会检测，归档或删除的站点会跳过且不会发送外部请求。

监控检测结果写入独立的 `monitoring_checks`，不会写入普通 `connectivity_tests`，因此不会影响首页测试历史、定时任务历史或统计页数据。通知策略固定为“恢复可用时通知”：首次检测成功会发送通知，连续成功不会重复发送；一旦后续检测失败或跳过，再次成功时会重新通知。
...
JSON 备份版本 7 会导出定时任务、监控任务定义和目标映射，但不会导出后台包装密钥、任务运行历史、监控检测历史或完整测试历史。导入时按分组名称以及站点名称 + Base URL 重新映射目标，所有导入的定时任务和监控任务都会保持停用，必须重新授权并由用户手动启用。旧版本 JSON 备份仍可继续导入。
