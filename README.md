...
- 首页支持“测试全部”，最多并发测试 5 个已启用中转站，并逐行更新最近结果。
...
JSON 默认备份包含代理名称、地址、启用状态和认证存在标记，但不包含用户名或密码。勾选“包含明文 API Key 和代理认证信息”并再次确认应用密码后，备份才会包含可用于完整恢复的敏感信息。请把此类备份视为明文密钥文件妥善保管。
